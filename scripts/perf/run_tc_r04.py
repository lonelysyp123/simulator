#!/usr/bin/env python3
"""TC-R04: EMU yt2 触发与监控同一点（FC6 保持寄存器），记录 T0/T1/ΔT。"""
import re
import subprocess
import time
...
def run_round(i: int, raw: int):
    print(f"\n--- 第 {i} 轮 (raw={raw}, 物理≈{raw / SCALE:.3f}) ---")
    write_reg(EMU_PORT, ADDR, SLAVE, RESET_RAW)
    time.sleep(0.5)
    baseline = read_reg(EMU_PORT, ADDR, SLAVE)
    print(f"  基线 {LABEL}(slave{SLAVE}@{ADDR})={baseline}")

    t0_str = write_reg(EMU_PORT, ADDR, SLAVE, raw)
    t0_ms = now_ms()
    print(f"  T0 触发写入 @ {t0_str}  ({LABEL}={raw})")

    t1, dt, val = wait_reg(t0_ms, EMU_PORT, ADDR, SLAVE, raw)
...
def main():
    print(f"TC-R04 自动化 (EMU {LABEL} 写入 → 同点监控, FC6/-t 4)")
    print(f"时间 {datetime.now().isoformat(timespec='seconds')}")

    v = read_reg(EMU_PORT, ADDR, SLAVE)
